...
using HelperOfDmr;
public class WillowSpider : MonoBehaviour,IMobSerializer,IMapGetSerializer
{
    EnemyMainStat EMS;
    ...
    private void Start()
    {
        ProceduralModuleGenerator.lastIndexFired += seralizeMe;
    ...
    public void Update()
    {
        if (EMS.EHC.Parried != LocalParried)
        {
            ...
            Stunned = randomStun;
            Target = null;
            AnimManager.Play("SpiderLabDeath");
    ...
    public void TakeDmg(bool Critical)
    {
        if (Critical)
        {
            ...
            if (this.enemyState == States.Attacking)
            {
                ...
                //Spider is resistive aganist stunning
            }
        }
        else
        {
    ...
    public void Die()
    {
        ...
        if (rand <= 50) PickableItemCreator.instance.CreatePickUpAbleItem(42, 1, this.transform, 5);
    ...
    public void AttackPlayer()
    {
        AttackParried = false;
        attack_Cooldown += AttackCooldown;
        AnimManager.Play("AttackSpiderLab");
        Invoke("OpenParryWindow", 0.30f);
        Invoke("CloseParryWindow", 0.45f);
    ...
    private void ClearAnim()
    {
        ...
        int random_ = Random.Range(0, 101);
        if (random_ <= 50)
        {
            PostProcessingManager.instance.gameObject.GetComponent<DrinkableStatPotionManager>().AddNewScriptPotion(41, 1000);
        }
        EMS.EHC.HitPlayer();
        this.enemyState = States.Chasing;
    }
    ...
    public void seralizeMe(int index)
    {
        ProceduralModuleGenerator.lastIndexFired -= seralizeMe;
